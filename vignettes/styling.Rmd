...
cqtkit 2.0.0 removes `set_style()`, `style_plot()`, and plain-list `style`
arguments; calling them errors with a pointer to the replacement.
`legend_location` and `conf_int` are deprecated and still work with a warning.
`style_spec()`, `legend_spec()`, `reveal()`, and `restyle_plot()` are
...
| `legend_location = "bottom"` | `style_spec(legend.position = "bottom")` |
| `conf_int = 0.95` | `show_model_results = model_results_spec(ci = 0.95)` |
