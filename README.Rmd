...
`cqtkit` provides preprocessing functions to help compute derived ECG parameters during data assembly. `preprocess()` computes the baseline-from-mean deltas and the row-level deltas from columns that must already be on the data:
...
Corrected QT is nonlinear in RR, so `preprocess()` does not derive `QTCB` and `QTCF` from replicate-averaged `QT` and `RR`. Compute them per replicate with `compute_qtcb_qtcf()` before averaging, and add the population baseline means with `compute_blm()`. See `vignette("data-assembly")`.

The included `cqtkit_data_<drug>` datasets are already fully derived, so the example below drops the delta columns first to show what `preprocess()` adds.
